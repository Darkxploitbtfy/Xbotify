'use strict';
const { addWarning, resetWarnings, getWarningCount } = require('../utils/dataManager');
...
async function handle({ sock, from, msg, isGroup, isAdmin }) {
  ...
  if (!isAdmin) {
    return sock.sendMessage(from, { text: '🔒 *Admin only.* You must be an admin to warn members.' });
  }
