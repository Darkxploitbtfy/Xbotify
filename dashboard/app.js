...
function createDashboard() {
  ...
  app.get('/panel',            (req, res) => {
    ...
    if (!isConnected())         return res.sendFile(path.join(__dirname, 'views', 'pairing.html'));
    return res.redirect('/panel/dashboard');
  });
  app.get('/panel/dashboard',  requireAuth, (_, res) => {
    if (!isConnected()) return res.redirect('/panel');
  ...
  app.post('/panel/api/pair', requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (result?.pairingCode) {
        return res.json({ success: true, code: result.pairingCode });
      }
  ...
  // ── User pairing (link a secondary device) ────────────
  app.post('/panel/api/pair-user', requireAuth, async (req, res) => {
    const { phone, days } = req.body;
    if (!phone) return res.json({ success: false, message: 'Phone number required.' });
    const clean = String(phone).replace(/\D/g, '');
    if (clean.length < 7) return res.json({ success: false, message: 'Invalid phone number.' });
    try {
      // Register the user in users.json with expiry FIRST
      addUser(clean, parseInt(days) || 30);
      // Then request a pairing code
      const result = await sessionManager.requestPairingCodeFor(clean);
      if (result?.alreadyConnected) {
        return res.json({ success: true, alreadyConnected: true });
      }
      if (result?.pairingCode) {
        return res.json({ success: true, code: result.pairingCode, phone: clean });
      }
      return res.json({ success: false, message: 'Failed to generate pairing code.' });
    } catch (e) {
