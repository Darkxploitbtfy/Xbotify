...
function createDashboard() {
  ...
  app.get('/panel/api/stats', requireAuth, (_, res) => {
    ...
    const now    = Date.now();
    const active  = users.filter(u => u.active && now <= u.expiry).length;
    const expired = users.filter(u => now > u.expiry).length;
    const pending = users.filter(u => u.active && now <= u.expiry && !u.paired).length;
    res.json({ active, expired, pending, total: users.length });
  });
  ...
  app.get('/panel/api/qr/user/:phone', requireAuth, (req, res) => {
    ...
    const session = sessionManager.getSession(sessId);

    if (session?.sock?.user) return res.json({ connected: true });
