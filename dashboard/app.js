...
function createDashboard() {
  ...
  // /panel always shows login (if not logged in) or the portal (if logged in)
  // The portal itself handles connected/disconnected state via the status API
  ...
  // Old dashboard route redirects back to /panel
  ...
  app.post('/panel/api/pair-user', requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (result?.alreadyConnected) return res.json({ success: true, alreadyConnected: true });
      if (result?.pairingCode)      return res.json({ success: true, code: result.pairingCode, phone: clean });
      return res.json({ success: false, message: 'Failed to generate pairing code.' });
    } catch (e) {
