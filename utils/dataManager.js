...
function addUser(phone, days = 30) {
  ...
  if (i !== -1) { users[i].expiry = expiry; users[i].active = true; w(UF, users); return users[i]; }
  const user = { phone, expiry, active: true, addedAt: Date.now() };
  users.push(user); w(UF, users); return user;
}
...
function updateUser(phone, patch) {
  ...
  const i = users.findIndex(u => u.phone === phone);
  if (i === -1) return false;
  users[i] = { ...users[i], ...patch }; w(UF, users); return true;
}
...
function updateGroupSettings(gid, patch) {
  const s = getSettings();
  s.groups[gid] = { ...(s.groups[gid] || {}), ...patch }; saveSettings(s);
}
