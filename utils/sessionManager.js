...
// ✅ FIX: destructure named export from groupParticipants
...
async function startSession({ id, phoneNumber = null, isOwner = false } = {}) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      if (isOwner) {
        // Always derive the owner number from the live socket user JID
  ...
  // ── Group member events (welcome / goodbye) ────────────
  ...
  if (!sock.authState.creds.registered && phoneNumber) {
    await new Promise((r) => setTimeout(r, 3000)); // let socket handshake
    try {
...
function restoreExistingSessions() {
  ...
  for (const e of entries) {
    ...
    tasks.push(
      startSession({ id, phoneNumber: phone, isOwner: id === OWNER_ID }).catch((err) =>
