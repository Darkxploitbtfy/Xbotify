...
async function startSession({ id, phoneNumber = null, isOwner = false } = {}) {
  ...
  // Per-session retry counter cache — fixes "Waiting for this message" on linked devices
  ...
  const sock = makeWASocket({
    ...
    // Prevents "Waiting for this message" — lets linked devices retry key fetches
    getMessage: async () => ({ conversation: '' }),
  });

  // Per-session feature toggles (antidelete, antiedit, anticall).
  // Each linked user controls their own bot independently.
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      if (isOwner) {
        ...
        if (num) {
          // Persist the owner number and update session.phoneNumber so
          // per-session features (mode, state) are keyed correctly.
  ...
  // Pass the session's own state so each user's antidelete toggle is respected.
...
async function requestPairingCodeFor(phoneNumber) {
  ...
  if (sessions.has(id)) {
    ...
    if (existing.sock?.user) return { id, alreadyConnected: true };
    existing.shuttingDown = true;
    try { existing.sock.end(undefined); } catch (_) {}
    sessions.delete(id);
  }
...
function restoreExistingSessions() {
  ...
  for (const e of entries) {
    const sid   = e.name;
    // For user sessions: phone is encoded in the directory name (user-XXXXXXXXX)
    // For owner session: phone is in config/env (will be set when connection opens)
    const phone = sid.startsWith('user-') ? sid.slice(5) : (config.owner.number || process.env.OWNER_NUMBER || null);
    tasks.push(
