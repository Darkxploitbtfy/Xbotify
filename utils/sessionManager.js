...
// Fetch the WA version with an 8-second timeout + hardcoded fallback.
// Prevents Railway crashes when GitHub is unreachable.
...
async function startSession({ id, phoneNumber = null, isOwner = false } = {}) {
  ...
  // ── Create session object first so Proxy closure can reference it ────
  ...
  // This is how antidelete / antiedit / anticall / alwaysonline survive
  // bot restarts — every assignment is intercepted and written to disk.
  const stateKey  = phoneNumber || id;
  const rawState  = { anticall: false, antidelete: false, antiedit: false, alwaysonline: false,
  ...
  const sessionState = new Proxy(rawState, {
    set(target, prop, value) {
      ...
      // alwaysonline timer managed here so command files stay simple
  ...
  // Start timer immediately if alwaysonline was persisted as true
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // Stop online timer before cleaning up session
  ...
  // ── Pairing code for new unregistered sessions ─────────────────────
...
async function _startUserSession(id, number) {
  try {
    if (sessions.has(id)) {
      ...
      if (existing.sock?.user) { pairingQueue.set(number, { alreadyConnected: true, done: true }); return; }
      existing.shuttingDown = true;
      try { existing.sock.end(undefined); } catch (_) {}
      sessions.delete(id);
    }
