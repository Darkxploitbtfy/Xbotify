...
async function startSession({ id, phoneNumber = null, isOwner = false } = {}) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      if (isOwner) {
        // Always sync the owner number from the live socket user JID
  ...
  sock.ev.on('messages.delete', async (update) => {
    try {
      ...
      logger.error({ err, id }, 'messages.delete handler failed');
    }
  });
  ...
  sock.ev.on('messages.update', async (updates) => {
    try {
      await handleMessageEdit(sock, updates);
    } catch (err) {
      logger.error({ err, id }, 'messages.update handler failed');
    }
  });
  ...
  sock.ev.on('call', async (calls) => {
    try {
      ...
      logger.error({ err, id }, 'call handler failed');
    }
  });
  ...
  if (!sock.authState.creds.registered && phoneNumber) {
    // Wait for the socket to complete its initial handshake
