...
async function handleMessages({ session, payload }) {
  ...
  // Per-session connected timestamp.
  // Each time the bot reconnects, session.connectedAt is updated to Date.now().
  // This ensures that messages sent while the bot was offline and delivered
  // in bulk on reconnect are discarded — preventing doubled responses.
  // A 15-second grace window covers messages sent right as the bot was
  // reconnecting (so real new messages are never accidentally dropped).
  ...
  for (const msg of messages) {
    try {
      ...
      // Drop messages that predate this socket's connection moment.
      // This fires on every reconnect (not just process restarts), which is
      // what prevents Baileys from replaying offline commands as doubled responses.
      ...
      // In newer Baileys / WhatsApp builds, message deletes and edits arrive via
      // messages.upsert as protocolMessage payloads, NOT the legacy messages.delete
      // / messages.update events.  Handle both delete (type 0 = REVOKE) and edit
      // (type 14 = MESSAGE_EDIT) here so antidelete / antiedit work for all clients.
      ...
      if (proto !== undefined && proto !== null) {
        ...
        if (proto.type === 0) {
          ...
          continue; // never expose revoke as a regular message
        }
        ...
        if (proto.type === 14) {
          if (state.antiedit) {
            ...
            if (originalId && newText) {
              const originalText = originalId ? editCache.get(originalId) : null;
        ...
        // All other protocol types (app-state sync, ephemeral settings, etc.)
        // are internal WA/Baileys housekeeping — not user-visible messages.
        continue;
      }
