...
// In-memory runtime state per process (resets on restart — by design)
const state = { anticall: false, antidelete: false, antiedit: false };

// Cache for antidelete/antiedit (DMs only, per message id)
...
async function handleMessages({ session, payload }) {
  ...
  // The phone number of whoever linked this WhatsApp session.
  // For the owner session this equals the global admin number.
  // For user sessions this equals the user's own phone number.
  ...
  for (const msg of messages) {
    try {
      ...
      // fromMe:true means the session owner typed this on their phone.
      // Two interesting cases:
      //   A) They typed a *command → process it (mapped to session owner)
      //   B) They replied to someone's status → save that status
      if (msg.key.fromMe) {
        const ctx = msg.message?.extendedTextMessage?.contextInfo;
        if (ctx?.remoteJid === 'status@broadcast') {
          // Session owner replied to a status → save it to their saved-messages
      ...
      // ── Secret view-once: emoji-only reply to a view-once ──────
      // Works for ALL linked users — sends the media to their own
      // "message yourself" chat (not hardcoded to admin).
      ...
      if (replyCtx?.quotedMessage && body.trim() && EMOJI_RE.test(body.trim())) {
        ...
        if (isQuotedVO) {
          // Send to the session owner's own number (their "message yourself")
          const destJid = sessionOwnerPhone + '@s.whatsapp.net';
          const fakeMsg = {
            key: {
              remoteJid:   from,
          ...
          await vvCmd.handleSecret(sock, fakeMsg, destJid);
          continue;
        }
      }
      ...
      // ── Resolve who is actually sending this command ────────────
      // For fromMe:true, the sender is the session owner (whoever linked
      // this WhatsApp). We use session.phoneNumber so each user's commands
      // are attributed to them — NOT always the global admin.
      const effectiveSender = msg.key.fromMe
        ? (sessionOwnerPhone + '@s.whatsapp.net')
        : (sender || from);

      // ── Access gate ─────────────────────────────────────────────
      // Passes session owner phone so each user's mode setting is respected.
      ...
      const ctx = {
        ...
        // Pass the session owner's phone so commands like *mode know
        // whose settings to update.
        sessionOwnerPhone,
        state,
      };
