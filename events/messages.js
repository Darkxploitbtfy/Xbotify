...
// Cache for antidelete (private chats, TTL 5 min)
...
async function handleMessages({ session, payload }) {
  ...
  for (const msg of messages) {
    try {
      ...
      if (!isGroup && state.antidelete && body) {
      ...
      if (isGroup && sender) {
        ...
        if (gs.antilink && LINK_RE.test(body)) {
          ...
          const count = addWarning(from, sender.split('@')[0]);
          if (count >= 5) {
            await sock.sendMessage(from, {
              text: `⚠️ @${sender.split('@')[0]} removed for repeated links.`,
              mentions: [sender]
            });
            try { await sock.groupParticipantsUpdate(from, [sender], 'remove'); } catch {}
            resetWarnings(from, sender.split('@')[0]);
          } else {
            await sock.sendMessage(from, {
              text: `⚠️ @${sender.split('@')[0]}, no links allowed! Warning ${count}/5`,
              mentions: [sender]
            });
          }
          continue;
      ...
      // Access check
      const access = checkAccess(sender || from);
      if (!access.allowed) {
        await sock.sendMessage(from, { text: denyMsg(access.reason) });
        continue;
      }

      const ctx = { sock, msg, from, sender, args, argStr, isGroup, isAdmin: access.isAdmin, state };
