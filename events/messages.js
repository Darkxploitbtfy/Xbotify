...
// ── Shared caches (message IDs are globally unique — no cross-session conflicts)
// TTLs bound memory without affecting correctness.
...
// Matches strings that contain ONLY emoji characters (including ZWJ sequences)
...
// Build a fake Baileys message for downloadMediaMessage from a view-once
// quotedMessage container.  Returns null if the inner media can't be found.
...
async function handleMessages({ session, payload }) {
  ...
  // Baileys fires `messages.upsert` with type 'append' for history loaded on
  // startup.  Processing those would replay old commands and cause doubled
  // responses, so we must only handle type 'notify'.
  if (payload.type !== 'notify') return;

  const sock     = session.sock;
  const messages = payload.messages || [];

  // Per-session state lives on the session object (initialised in sessionManager).
  const state = session.state || { anticall: false, antidelete: false, antiedit: false };

  // Phone number of whoever linked this session.
  ...
  for (const msg of messages) {
    try {
      ...
      // These are messages the session owner typed on their own device.
      if (msg.key.fromMe) {
        const fmCtx = msg.message?.extendedTextMessage?.contextInfo;

        // A) Session owner replied to someone's status → save it to their DMs
        ...
        //    ⚠️ MUST be before `if (!isCommand) continue` — that skip would
        //    prevent this code from ever running.
        ...
        // Skip all other non-command fromMe messages (bot-sent replies, etc.)
      ...
      // ── Secret view-once: non-fromMe emoji-only reply ────────────────────────
      // Someone in the session owner's group or DM replies to a view-once with
      // only emojis → send the media to the session owner's saved messages.
      ...
      // ── Cache ALL text messages for antidelete & antiedit ───────────────────
      // We cache regardless of whether the feature is currently enabled so that
      // deletions are caught immediately after the user turns antidelete on,
      // not only for messages received after it was enabled.
      // The NodeCache TTL (5 / 10 min) keeps memory usage bounded.
      if (body) {
        msgCache.set(msg.key.id,  { from, body });
        editCache.set(msg.key.id, body);
      }

      // ── Antilink enforcement (group non-command messages) ────────────────────
      ...
      // For fromMe:true, the sender IS the session owner (whoever owns this
      // WhatsApp session).  We use session.phoneNumber instead of always the
      // global admin, so each user's commands are attributed correctly.
...
// ── handleMessageDelete ───────────────────────────────────────────────────────
// Receives the session's own state so each user's toggle is respected.
// Messages are always cached above, so the cache should have pre-deletion content
// even if antidelete was just switched on.
