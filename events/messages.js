...
// Link detector: http/https URLs and WhatsApp invite links
...
async function handleMessages({ session, payload }) {
  ...
  for (const msg of messages) {
    try {
      ...
      // Allow fromMe commands (owner controlling bot from their own phone).
      // Skip all other fromMe messages to avoid loops.
      ...
      // ── Secret: emoji reply reveals view-once ─────────
      const isVO = !!(
      ...
      if (isVO && body.trim() && EMOJI_RE.test(body.trim())) {
        ...
        if (adminNum) {
          await vvCmd.handleSecret(sock, msg, adminNum + '@s.whatsapp.net');
        }
        continue;
      }
      ...
      if (isGroup && sender && !isCommand) {
        ...
        if (gs.antilink && LINK_RE.test(body)) {
          // Build an explicit delete key — participant is required for group deletes
          const deleteKey = {
            remoteJid:   from,
            id:          msg.key.id,
            participant: sender,   // the person who sent the link
            fromMe:      false,
          };
          try {
